// filepath: /home/zarif/Project/taxmin/front-end/src/pages/admin/learning/AddSubchapter.jsx
...
const AddSubchapter = () => {
    ...
    const fetchChapter = async () => {
      try {
        ...
        if (response.status === 'success') {
          ...
          const subchapterCount = response.data.subchapters?.length || 0;
          setFormData(prev => ({ ...prev, orderIndex: subchapterCount + 1 }));
        }
      } catch (error) {
        showNotification("error", error.message || "Gagal memuat data chapter");
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await subchapterService.createSubchapter({
        ...formData,
        chapterId: parseInt(chapterId)
      });
      
      if (response.status === 'success') {
        showNotification("success", "Subchapter berhasil ditambahkan!");
        setTimeout(() => {
          navigate(`/admin/subchapters/${chapterId}`);
        }, 1500);
      }
    } catch (error) {
      showNotification("error", error.message || "Gagal menambahkan subchapter!");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center space-x-4">
        ...
        <div>
          <h1 className="text-3xl font-bold text-primary">Tambah Subchapter</h1>
          <p className="text-secondary mt-1">Buat subchapter baru untuk chapter ini</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-md p-6 space-y-6">
        ...
        <div>
          ...
          <p className="text-xs text-secondary mt-1">
            Urutan bisa diubah dengan drag & drop setelah subchapter dibuat
          </p>
        </div>
        ...
        <div className="flex gap-4 pt-4">
          ...
          >
            {loading ? 'Menyimpan...' : 'Simpan Subchapter'}
          </button>
        </div>
      </form>
